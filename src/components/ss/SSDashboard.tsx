...
export const SSDashboard: React.FC<SSDashboardProps> = ({
  ...
  const sixUpsPlansCount = plans.filter(p => p.ups === 6).length;
  const sixUpsPct = plans.length > 0 ? Math.round((sixUpsPlansCount / plans.length) * 100) : 100;
    ...
    <div className="space-y-6" id="ss-dashboard-view">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs">
          <div className="flex items-center justify-between text-slate-500 mb-2">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-600">6-UPS Policy Target</span>
            <TrendingUp className="w-4 h-4 text-emerald-600" />
          </div>
          <div className="text-2xl font-black text-emerald-700 font-mono">
            {sixUpsPct}% <span className="text-xs font-sans text-slate-500 font-normal">Preference</span>
          </div>
          <div className="mt-2 flex items-center justify-between text-xs text-slate-600">
