...
import { SSPlanDetailViewer } from './SSPlanDetailViewer';
...
export const SSJumboRequirementsView: React.FC<SSJumboRequirementsViewProps> = ({
  ...
  const handleSendSelectedToMSL = () => {
    ...
    try {
      ...
      saveStoredJumboRolls([...existingRolls, ...dedupedNewRolls]);
  ...
  return (
    <div className="space-y-6" id="ss-requirements-view">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs">
                ...
                <div className="text-2xl font-black text-slate-800 font-mono mt-1">
                  {filmOrders.reduce((sum, o) => sum + (o.remaining_qty * SS_CUSTOMER_MAX_OVERALLOCATION_FACTOR), 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} <span className="text-xs font-normal text-slate-500 font-sans">KG</span>
                </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-600 border-b border-slate-200">
                  ...
                  <th className="py-2.5 px-3 font-bold">SS PLAN(S)</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 font-mono">
                {filmOrders.map(ord => {
                  ...
                  const maxAllowedKg = Number((ord.remaining_qty * SS_CUSTOMER_MAX_OVERALLOCATION_FACTOR).toFixed(2));
                  ...
                  // 1. Identify all physical SS plans that cover this exact Sales Order + Item
                  ...
                  filteredReqs.forEach((req, originalIndex) => {
                    ...
                    if (isMatch) {
                      matchedReqPlans.push({
                        planNumber: `SS PLAN #${originalIndex + 1}`,
                        req,
                        originalIndex
                      });
                    }
                  ...
                  return (
                    <tr key={ord.id} className="hover:bg-slate-50/60">
                      ...
                      <td className="py-2 px-3 text-right font-bold">
                        <span className={overagePct > 10 ? 'text-rose-600' : overagePct > 0 ? 'text-purple-700' : 'text-slate-400'}>
                          ...
                          <div className="flex flex-wrap items-center gap-1.5">
                            {distinctPhysicalPlans.map(plan => (
                              <button
                                ...
                                onClick={() => setSelectedPhysicalSsPlan(plan)}
                                ...
                                title={`Open SS Plan ${plan.plan_number}`}
                              >
      ...
      {/* Complete Full Authoritative Physical Secondary Slitter Sheet Modal */}
      {selectedPhysicalSsPlan && (
        <SSPlanDetailViewer
          plan={selectedPhysicalSsPlan}
          currentUser={defaultPlannerUser}
          onClose={() => setSelectedPhysicalSsPlan(null)}
        />
      )}
