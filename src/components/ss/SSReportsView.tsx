...
export const SSReportsView: React.FC<SSReportsViewProps> = ({
  ...
  const sixUpsPct = plans.length > 0 ? Math.round((plans.filter(p => p.ups === 6).length / plans.length) * 100) : 100;
      ...
      {activeReportTab === 'efficiency' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs">
              <span className="text-xs font-bold uppercase text-slate-500">6-UPS Policy Target</span>
              <div className="text-2xl font-black text-slate-900 font-mono mt-1">
                {sixUpsPct}% <span className="text-xs font-sans text-slate-500 font-normal">Compliant</span>
              </div>
            </div>
          </div>
