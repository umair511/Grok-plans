...
import { isSSFilm } from '../../services/ss/ssOptimizer';
import { 
...
const SS_FILM_OPTIONS = FILM_MASTERS.filter(m => isSSFilm(m.code));
const DEFAULT_SS_FILM = SS_FILM_OPTIONS[0]?.code || 'TNO20';
...
export const SSJumboInventoryView: React.FC<SSJumboInventoryViewProps> = ({
  ...
  const [importText, setImportText] = useState(`TNO20\t1700\t39000\t20\t6" paper
TNO18\t1650\t39000\t18\t6" paper
MATTPL12\t1700\t20000\t12\t6" paper
TNBPL10\t1650\t20000\t10\t6" paper
TNIT-23\t1700\t16900\t23\t6" paper`);
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setEditFilm(code);
                      const master = SS_FILM_OPTIONS.find(m => m.code === code);
                      if (master) setEditThickness(master.thickness_micron);
                    }}
                  ...
                  >
                    {SS_FILM_OPTIONS.map(m => (
                      <option key={m.code} value={m.code}>
                        {m.code} ({m.thickness_micron}µm · {m.category})
                      </option>
                    ))}
                    {editFilm && !SS_FILM_OPTIONS.some(m => m.code === editFilm) && (
                      <option value={editFilm}>{editFilm}</option>
                    )}
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 text-xs bg-slate-50 border border-slate-300 rounded-lg font-mono focus:ring-2 focus:ring-purple-500"
                    placeholder="e.g. JR-TNO20-1700-01"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const master = SS_FILM_OPTIONS.find(m => m.code === code);
                      if (master) {
                        setNewThickness(master.thickness_micron);
                        setNewRollId(`JR-${code}-${Date.now().toString().slice(-4)}`);
                      }
                    }}
                    className="w-full px-3 py-1.5 text-xs bg-slate-50 border border-slate-300 rounded-lg font-semibold text-purple-800 focus:ring-2 focus:ring-purple-500"
                  >
            ...
            <div className="space-y-4 my-4 overflow-y-auto pr-1 flex-1">
              ...
              <div>
                <textarea
                  ...
                  placeholder={'TNO20\t1700\t39000\t20\t6" paper'}
                />
              </div>
